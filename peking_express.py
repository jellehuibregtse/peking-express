...
class PekingExpress:
    ...
    def updated_occupied_locations(self):
        if len(self.occupiedLocations) > self.currentTurn:
            self.occupiedLocations[self.currentTurn] = self.occupiedLocations[self.currentTurn] + [
                self.character.path[-1]]
        else:
    ...
    def next_move(self):
        ...
        if shortest_path[0] != shortest_path[1]:
            self.character.spent = self.character.spent + self.pekingMap.get_vertex(shortest_path[0]).weight(
                shortest_path[1])
